...
The live site and the headless project are two separate Wix properties. **Identify them by ID, never
by console display name:**

| | Property | Fixed ID | Role |
|---|---|---|---|
| **P_A** | Legacy Wix Editor site | metaSiteId `9dbcaac9-3336-4ec2-8c8f-4a4f275aad06` | Live now; **keep** = rollback target |
| **P_B** | Headless project | siteId `9830980c-d83c-48fd-aa0c-d67c909406bd` | Deploy target (= `wix.config.json`) |

Confirm P_A from the live domain itself:
...
data on P_A to preserve — zero Contacts, zero Site Members beyond account-level admin logins, and
...
4. Reassign the domain back to P_A. A domain maps to one property at a time; both can be Premium at
   once. This is a within-Wix reassignment with an SSL re-validation window, not a DNS TTL flip.

### Prerequisites

- **[BLOCKER] Consolidate P_A, the domain, and P_B into the org owner's account.** Domain reassignment
  (*Domains → Assign to a Different Site*) only lists sites **within the account holding the domain**.
  Today the domain and P_A are in the org owner's account and P_B is in the operator's, so the swap
  cannot be done as-is. Direction: bring **P_B to the org** (*Transfer a Premium Site to Another Wix
  Account*), not P_A to the operator — the org keeps ownership of its web presence. `.org` transfers
  freely, and the metaSite id is stable across account moves, so the repo keeps deploying to the same
  P_B. The org owner then re-invites the operator as co-owner so they remain the technical deployer.
- **[VERIFY] Does co-owner/admin suffice for `release`/`preview`?** The deploy path was only ever
  proven as the account *owner* of P_B, and the blocking permission is described as project-owner
  level. Stage II settles this.
- **Premium plan on P_B, bought in the org account** (after the transfer) — Wix-Managed Headless needs
  one to serve a custom domain, and Premium is per-site.

### Execution — three stages

Blue = P_A (legacy, live), green = P_B (headless). Prove green under progressively less privilege,
then hand the domain switch to the org.

- **Stage I — deploy to green as OWNER.** While the operator still owns P_B: get the deployable state
  onto `main` (including banner removal), `deploy:preview`, validate (§4), `deploy:release`.
  *Exit:* a validated, released P_B. No org involvement.
- **Stage II — deploy to green as ADMIN (co-owner).** Prove the untested assumption above. Cheapest
  faithful test is a colleague-sandbox round-trip: transfer P_B to a trusted colleague → they re-invite
  the operator as co-owner → operator runs `deploy:preview` → colleague transfers P_B back. The wall is
...
- **Stage III — transfer green into the org's account for the domain switch.** Only after I and II
  pass. Then the org's officers reassign the domain P_A→P_B at their discretion — a subdomain
  (`beta.repac-riverside.org`) first, then the apex — with rollback = reassign back to P_A.
